...
- **幂等只入账一次（且按内容绑定）**：预收/支出/冲销都要求 `idempotency_key`，唯一索引保证重复提交、网络重试、并发同键只有一笔入账，
  其余返回首次结果（响应 `replayed=true`）。前端使用 `crypto.randomUUID()` 生成。
  回放前逐项比对请求内容与已入账明细——预收/支出要求**案件、客户、资金类型、金额**完全一致，
  冲销要求**冲销对象**一致；任一字段不同即视为幂等键被不同请求复用，返回 409 / 40906 拒绝，**绝不回放无关明细**。
